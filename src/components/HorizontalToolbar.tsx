...
type ToolDef = { id: ToolType; label: string; key: string; icon: React.ReactNode };
...
export const HorizontalToolbar: React.FC<Props> = ({ tool, locked, onToolChange, onToggleLock }) => (
  <div className="htoolbar">
    {/* Lock button */}
    <button
      className={`htool-btn${locked ? ' active' : ''}`}
    ...
    <div className="htool-sep" />

    {TOOLS.map(t => (
      <button
        key={t.id}
        className={`htool-btn${tool === t.id ? ' active' : ''}`}
        title={`${t.label} (${t.key})`}
        onClick={() => onToolChange(t.id)}
      >
        {t.icon}
        <span className="htool-key">{t.key}</span>
      </button>
    ))}
  </div>
);
